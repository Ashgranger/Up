...
async def ws_engine(stop: asyncio.Event):
    ...
    - New ClientSession per connection attempt (avoids stale session state)
    - Single receive loop with manual PING every WS_PING_INTERVAL seconds
    - Outgoing subscribe/unsubscribe messages drained inline each iteration
    ...
    while not stop.is_set():
        ...
        try:
            ...
            # Re-subscribe to any tokens already tracked
            if _subscribed:
                ...
                log.info(f"WS re-subscribed {len(_subscribed)} tokens")

            last_ping = time.time()

            while not stop.is_set():
                # Wait up to 5s for a message; timeout is normal (just means idle)
                try:
                    wsmsg = await asyncio.wait_for(ws.receive(), timeout=5.0)
                except asyncio.TimeoutError:
                    wsmsg = None
                except Exception as recv_err:
                    log.error(f"WS receive error: {recv_err}")
                    break
                ...
                if now - last_ping >= WS_PING_INTERVAL:
                    try:
                        ...
                        log.error(f"WS PING failed: {ping_err}")
                        break

                # Drain outgoing queue (subscribe/unsubscribe from main loop)
                while not _ws_out.empty():
                    try:
                        out_msg = _ws_out.get_nowait()
                        await ws.send_str(out_msg)
                        log.debug(f"WS sent: {out_msg[:80]}")
                    except asyncio.QueueEmpty:
                        break
                    except Exception as send_err:
                        log.error(f"WS send error: {send_err}")
                        break

                if wsmsg is None:
                    continue  # timeout — PING/drain done above, loop again
                ...
                if wsmsg.type == aiohttp.WSMsgType.TEXT:
                    ...
                    log.warning(f"WS closed/error: type={wsmsg.type} data={wsmsg.data}")
                    break
        ...
        if not stop.is_set():
            log.info(f"WS sleeping {WS_RECONNECT_DELAY}s before reconnect...")
