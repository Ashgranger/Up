...
async def run_cycle(session: aiohttp.ClientSession, cycle_num: int, next_ts: int = 0):
    if not next_ts:
    ...
    # Clear any stale price state for these tokens before subscribing
    prices.clear(market.up_token)
    prices.clear(market.down_token)

    # Register tokens with WS engine
    await subscribe([market.up_token, market.down_token])
    log.info("  Waiting for real prices (WS + REST, up to 30s)...")

    # Poll until real prices on BOTH legs or 30s timeout.
    # Market orderbook may seed slowly — REST every 5s as backup.
    ...
    while time.time() < deadline:
        ...
        if 0 < ua < 1.0 and 0 < da < 1.0:
            log.info(f"  WS prices: UP bid={ub:.4f} ask={ua:.4f}  DOWN bid={db:.4f} ask={da:.4f}")
            break
        if time.time() - last_rest >= 5.0:
            await fetch_prices_rest(session, market)
            last_rest = time.time()
            ub, ua = prices.get(market.up_token)
            db, da = prices.get(market.down_token)
            if 0 < ua < 1.0 and 0 < da < 1.0:
                log.info(f"  REST prices: UP bid={ub:.4f} ask={ua:.4f}  DOWN bid={db:.4f} ask={da:.4f}")
                break
    else:
        ub, ua = prices.get(market.up_token)
        db, da = prices.get(market.down_token)
        log.warning(f"  30s timeout — no real prices yet (UP ask={ua:.4f} DOWN ask={da:.4f})")
...
async def main():
    ...
    async with aiohttp.ClientSession(connector=connector) as session:
        ...
        try:
            while True:
                # Always compute the next market fresh here — avoids stale ts
                # from a previous cycle that ran long or errored.
                next_ts = next_market_start()
                log.info(f"  → Next market: {slug_for(next_ts)}")
                try:
                    await run_cycle(session, cycle, next_ts)
                except asyncio.CancelledError:
                    raise
                except Exception as e:
